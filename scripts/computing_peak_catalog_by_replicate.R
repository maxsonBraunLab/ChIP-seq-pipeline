...
### strip replicate information as 'treatment' col.
filtered_peaks <- filtered_peaks %>% 
    mutate(treatment = str_replace(name, "_peak.+", "")) %>% 
    mutate(treatment = str_replace(treatment, "._", "_"))
...
# import bed file with all peaks concatenated by replicate
# create 'treatment' col with replicate info stripped out.
# rm blacklist 
# select canonical chr
# split table into multiple subclasses ($) by treatment
# for each treatment, compute_coverage() output will give number of overlaps of reps. We filter for intervals with >= "presence_in_samples" + rm blacklist. This mini-table is 'coverage', and it gets added to 'catalog'. 'coverage' is exported in addition to 'catalog'.
# if there are intersections among the reps, recalc seq len and add that to the union 'catalog'.
# export 'catalog'.